...
function tapIfElementValid(name) {
    var element = Finder.findElementByName(name);
        // Finder.findElementByNameAndClassType("Cancel","Button").tap();
        if (!Finder.isNil(element))
            element.tap();
}

function tapIfElementValid(name, classType) {
    var element = Finder.findElementByNameAndClassType(name,classType);
        // Finder.findElementByNameAndClassType("Cancel","Button").tap();
        if (!Finder.isNil(element))
            element.tap();
}
...
function diffPNG(image1,image2){
    ...
    var result = host.performTaskWithPathArgumentsTimeout("/usr/bin/diff", [image1,image2], 5);
    if(result.exitCode==0){
        return true;
    }
    return false;
